...
RUN --mount=type=cache,id=${CACHE_KEY}-apt-cache,target=/var/cache/apt,sharing=shared \
  --mount=type=cache,id=${CACHE_KEY}-apt-lists,target=/var/lib/apt/lists,sharing=shared \
  apt-get update \
  && apt-get upgrade -y \
...
COPY . ./
RUN --mount=type=cache,id=${CACHE_KEY}-next-cache,target=/app/.build/next/cache \
