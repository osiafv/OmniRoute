...
RUN --mount=type=cache,id=jytvid-apt-cache,target=/var/cache/apt,sharing=shared \
  --mount=type=cache,id=jytvid-apt-lists,target=/var/lib/apt/lists,sharing=shared \
  apt-get update \
  && apt-get upgrade -y \
...
# broken image.
RUN --mount=type=cache,id=jytvid-npm-cache,target=/root/.npm \
...
COPY . ./
RUN --mount=type=cache,id=jytvid-next-cache,target=/app/.build/next/cache \
...
RUN --mount=type=cache,id=jytvid-apt-cache-locked,target=/var/cache/apt,sharing=locked \
  --mount=type=cache,id=jytvid-apt-lists-locked,target=/var/lib/apt/lists,sharing=locked \
  apt-get update \
